...
function MobileSection() {
  ...
  const applyXStyle = useCallback((el: HTMLElement | null, progress: number, index: number) => {
    if (!el) return;
    const start = index / TOTAL, end = (index + 1) / TOTAL;
    const enter = start + 0.1, exitS = end - 0.1;
  ...
  const onScroll = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      dotCoreRefs.current.forEach((core, i) => {
        ...
        core.style.backgroundColor = (progress >= s + 0.01 && progress < e - 0.01) ? "#ffffff" : "#6b7280";
      });

      dotGlowRefs.current.forEach((glow, i) => {
        if (!glow) return;
        const s = i / TOTAL, e = (i + 1) / TOTAL;
        const sG = s + 0.05, eG = e - 0.05;
        let op = 0;
        if (progress >= s && progress < e) {
          if (progress < sG)      op = mapRange(progress, s, sG, 0, 1);
          else if (progress < eG) op = 1;
          else                    op = mapRange(progress, eG, e, 1, 0);
        }
  ...
  const scrollToStep = (index: number) => {
    const section = sectionRef.current;
    if (!section) return;
    const absTop = window.scrollY + section.getBoundingClientRect().top;
      ...
      <div className="sticky top-0 h-svh flex items-center justify-center overflow-hidden px-5 py-8">
        ...
        */}
        <div className="w-full flex flex-col">
          ...
          {/* ── Title ── */}
          <h2
            className="font-serif font-normal whitespace-nowrap flex-shrink-0"
            style={{
          ...
          <div
            className="flex-shrink-0"
            style={{ position: "relative", width: "100%", height: 2, background: "#2a3441", borderRadius: 99 }}
          >
            {/* Fill */}
            <div
              ref={fillRef}
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              {steps.map((_, i) => (
                <div
                  ...
                  style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, cursor: "pointer" }}
                >
                  {/* Glow */}
                  <div
                    ...
                    style={{
                      ...
                      opacity: i === 0 ? 1 : 0, transition: "opacity 0.15s",
                    }}
                  />
                  {/* Core */}
                  <div
